...
class _TodayOrderState extends State<TodayOrder> {
  ...
  Future<dynamic> printOrCreateInvoice(Invoice invoice, AppLocalizations locale) async {
    ...
    final profile = await CapabilityProfile.load();
    Generator ticket = new Generator(_paperSize, profile);
    List<int> bytes1 = [];
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    bytes1 += ticket.text('${invoice.address}', styles: PosStyles(align: PosAlign.center));
    bytes1 += ticket.text('${invoice.city} (${invoice.pincode})', styles: PosStyles(align: PosAlign.center));
    bytes1 += ticket.text('Tel: ${invoice.number}', styles: PosStyles(align: PosAlign.center));
    bytes1 += ticket.hr();
    bytes1 += ticket.row([
    ...
    for (OrderDetails details in orderDetaisl) {
      bytes1 += ticket.row([
        ...
        PosColumn(text: '${details.price.toStringAsFixed(2)}', width: 2, styles: PosStyles(align: PosAlign.right)),
        PosColumn(text: '${(details.price * double.parse('${details.qty}')).toStringAsFixed(2)}', width: 2, styles: PosStyles(align: PosAlign.right)),
      ]);
      it++;
    }
    ...
    bytes1 += ticket.row([
      ...
      PosColumn(text: '$apCurrency ${invoice.totalPrice.toStringAsFixed(2)}', width: 5, styles: PosStyles(align: PosAlign.right, width: PosTextSize.size2)),
    ]);
    bytes1 += ticket.row([
      PosColumn(text: locale.invoice6h, width: 7, styles: PosStyles(align: PosAlign.right, width: PosTextSize.size2)),
      PosColumn(text: '$apCurrency ${invoice.deliveryCharge.toStringAsFixed(2)}', width: 5, styles: PosStyles(align: PosAlign.right, width: PosTextSize.size2)),
    ]);

    bytes1 += ticket.feed(2);
    bytes1 += ticket.text(locale.invoice7h, styles: PosStyles(align: PosAlign.center, bold: true));

    final now = DateTime.now();
    final formatter = DateFormat('MM/dd/yyyy H:m');
    final String timestamp = formatter.format(now);
    bytes1 += ticket.text(timestamp, styles: PosStyles(align: PosAlign.center), linesAfter: 2);
    bytes1 += ticket.feed(2);
    bytes1 += ticket.cut();
    return bytes1;
  }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 16),
                    Text(locale.orderedOn + ' ${mainP.orderDetails[0].orderDate}', style: Theme.of(context).textTheme.subtitle2.copyWith(fontSize: 10.5)),
                  ],
                ),
              ),
            ],
