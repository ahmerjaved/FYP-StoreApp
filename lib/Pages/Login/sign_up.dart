                    ...
                    child: DropdownButton<CityDataBean>(
                      ...
                      items: cityList.map((value) {
                        return DropdownMenuItem<CityDataBean>(
                          value: value,
                          child: Text(value.cityName, overflow: TextOverflow.clip),
                        );
                      }).toList(),
                      onChanged: (value) {
                        setState(() {
                                  ...
                                  if (addressC.text != null) {
                                    if (idValue != null) {
                                      if (idNumberC.text != '') {
                                        if (_image != null) {
                                          if (idPhoto != null) {
                                            hitSignUpUrl(sellerNameC.text, storeNameC.text, phoneNumberC.text, emailAddressC.text, passwordC.text, cityData.cityName, adminShareC.text, deliveryRangeC.text, addressC.text, context, idValue.name, idNumberC.text);
                                          } else {
                                            setState(() {
                                              showDialogBox = false;
                                            });
    ...
    if (_image != null) {
      ...
      requestMulti.send().then((values) {
        values.stream.toBytes().then((value) {
          ...
          Toast.show(jsonData['message']!=null?'${jsonData['message']}':'Some error occurred.', context, gravity: Toast.CENTER, duration: Toast.LENGTH_SHORT);
          setState(() {
            showDialogBox = false;
          });
        }).catchError((e) {
